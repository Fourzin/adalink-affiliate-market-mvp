...
const SignUpWindow = ({ walletAPI,walletName,walletIcon,lucid,openWalletMenu,onClose,setLoggedIn,setAccountInfo,selectedIPID,selectedSPOID,setMessageWindowContent,setMessageWindowButtonText,setShowMessageWindow}) => {
  ...
  const [userType,setUserType] = useState("SPO");
  ...
  async function handleRegistration() {
    ...
    if(displayName.replace(/\s/g, "").length<3){
      setMessageWindowContent("Display name has to be longer than 3 charecters.");
    ...
    if(userType=="SPO"){
      if(projectDescription.replace(/\s/g, "").length<10){
        setMessageWindowContent("Add project description.");
        setMessageWindowButtonText('OK');
        setShowMessageWindow(true);
        return;
      }
  ...
  async function isAccountRegistered(userType,checkWith,checkValue){
    ...
    response = await fetch('https://adalink.io/api/is-account-registered-bz.php?userType='+userType+'&'+checkWith+'='+checkValue); 
        ...
        <div style={{display:"flex",gap:"30px"}}>
          <div className='sign-up-text-fields-area'>
            ...
            <div className='sign-up-text-field' style={{gap:"16px"}}>
              ...
              <div className='sign-up-text-field-input' >
                <select id="projectTypeR" style={{height:"25px"}} onChange={() => setProjectType(document.getElementById("projectTypeR").value)}>
