...
const EditWindow = ({ onClose,parameterToBeEdited,accountInfo,setAccountInfo,setMessageWindowContent,setMessageWindowButtonText,setShowMessageWindow}) => {
  ...
  async function handleParameterEdit() {
    ...
    let response = await fetch("https://adalink.io/api/edit-user-parameter-adalink-bz.php?stakeAddress="+stakeAddress+'&parameter='+parameterToBeEdited.replace(/\s/g, "")+'&value='+parameterNewValue+'&type='+userType+'&key='+key,{cache:'reload'});
